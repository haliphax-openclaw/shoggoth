...
2. Update the frontmatter in `README.md`: set `date` to today, `status` to `planned`, and `completed` to `never`.
...
Update `status` as work progresses. Set `completed` to the actual date when done.
...
1. Update the frontmatter: set `status` to `complete` and `completed` to today's date.
...
If a plan is no longer relevant, update the frontmatter: set `status` to `abandoned` and add a brief note at the top of the README explaining why. Move it to `plans/done/` so it's out of the active directory but preserved for reference.
