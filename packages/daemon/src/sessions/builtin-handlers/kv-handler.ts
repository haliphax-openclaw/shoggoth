...
function kvGet(
  ...
  if (!row) {
    return { resultJson: JSON.stringify({ ok: true, key, value: null, exists: false }) };
  }
  ...
  return { resultJson: JSON.stringify({ ok: true, key, value: parsed, exists: true }) };
}
...
function kvSet(
  ...
  if (key.length > MAX_KEY_LENGTH) {
    return { resultJson: JSON.stringify({ error: `key exceeds max length of ${MAX_KEY_LENGTH} chars` }) };
  }
  ...
  if (Buffer.byteLength(serialized, "utf8") > MAX_VALUE_BYTES) {
    return { resultJson: JSON.stringify({ error: `value exceeds max size of ${MAX_VALUE_BYTES} bytes when serialized` }) };
  }
...
function kvDelete(
  ...
  return { resultJson: JSON.stringify({ ok: true, key, deleted: info.changes > 0 }) };
}
